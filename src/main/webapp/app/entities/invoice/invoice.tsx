  ...
  return (
    <div>
      <h2 id="invoice-heading">
        <Translate contentKey="task0App.invoice.home.title">Invoices</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="task0App.invoice.home.createLabel">Create new Invoice</Translate>
        </Link>
      </h2>
        ...
        {invoiceList && invoiceList.length > 0 ? (
            ...
            <tbody>
              {invoiceList.map((invoice, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{invoice.contract ? <Link to={`contract/${invoice.contract.id}`}>{invoice.contract.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="task0App.invoice.home.notFound">No Invoices found</Translate>
            </div>
          )
        )}
      </div>
